...
func (t *LogFormatter) Format(entry *logrus.Entry) ([]byte, error) {
	...
	if entry.HasCaller() {
		// 自定义文件路径
		funcVar := entry.Caller.Function
		fileVal := fmt.Sprintf("%s:%d", path.Base(entry.Caller.File), entry.Caller.Line)
		// 自定义输出格式
		fmt.Fprintf(b, "%s [%s] \x1b[%dm[%s]\x1b[0m %s %s %s\n", global.Config.Logger.Prefix, timestamp, levelColor, entry.Level, fileVal, funcVar, entry.Message)
	} else {
...
func InitLogger() *logrus.Logger {
	...
	// 输出到文件
	file, _ := os.OpenFile("service_log.log", os.O_CREATE|os.O_APPEND|os.O_WRONLY, 0666)
